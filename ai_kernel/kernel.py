...
# Отдельный, БОЛЬШИЙ бюджет для случая, когда локальная модель синтезирует
# ответ из результатов поиска (это законно длиннее короткой болталки). 600
# токенов тут обрезали ответ про матч на полуслове (finish_reason=length), и
# пользователь получал заглушку «не успел». ~1200 при таймауте 25с и ~55 ток/с
# укладывается в один раунд синтеза.
LOCAL_TOOLS_MAX_TOKENS = 1200
...
# --- Очистка ответа от markdown/эмодзи ------------------------------------
# Локальная qwen3 (и иногда облако) упорно суют в ответ **жирный**, ##-заголовки,
# списки и эмодзи, несмотря на прямой запрет в system_prompt. Для чата, а тем
# более для будущего голосового интерфейса, это мусор. Полагаться на промпт
# бесполезно (проверено логами: игнорирует) — чистим финальный текст механически.
_EMOJI_RE = re.compile(
    "["
    "\U0001F000-\U0001FAFF"  # эмодзи, пиктограммы, символы, транспорт, флаги и т.п.
    "\U00002600-\U000027BF"  # разные символы + dingbats
    "\U00002190-\U000021FF"  # стрелки
    "\U00002B00-\U00002BFF"  # доп. символы и стрелки
...
def _sanitize_reply(text: str) -> str:
    """Убирает markdown-разметку и эмодзи из ответа модели.

    НЕ трогает обычную типографику (— … «» — она нужна в русском тексте),
    вычищает только то, что модель суёт как разметку.
    """
    if not text:
        return text
    text = re.sub(r"```[^\n]*\n?", "", text)         # блоки кода ```
    text = text.replace("`", "")                      # инлайн-код
    text = re.sub(r"\[([^\]]+)\]\((https?://[^)\s]+)\)", r"\1 (\2)", text)  # [текст](url)
    text = re.sub(r"(?m)^\s{0,3}#{1,6}\s*", "", text)  # ##-заголовки
    text = re.sub(r"(?m)^\s{0,3}([-*_])\1{2,}\s*$", "", text)  # --- *** ___
    text = re.sub(r"\*\*(.+?)\*\*", r"\1", text, flags=re.DOTALL)  # **жирный**
    text = re.sub(r"__(.+?)__", r"\1", text, flags=re.DOTALL)      # __жирный__
    text = re.sub(r"(?<![\w*])\*(?!\s)(.+?)(?<!\s)\*(?![\w*])", r"\1", text, flags=re.DOTALL)  # *курсив*
    text = text.replace("**", "").replace("__", "")   # осиротевшие маркеры
    text = re.sub(r"(?m)^\s{0,3}[-*+]\s+", "— ", text)  # маркеры списков -> «— »
    text = _EMOJI_RE.sub("", text)                    # эмодзи
    text = re.sub(r"[ \t]{2,}", " ", text)            # двойные пробелы (от вырезанных эмодзи)
    text = re.sub(r"(?m)[ \t]+$", "", text)           # хвостовые пробелы
    text = re.sub(r"(?m)^[ \t]+", "", text)           # ведущие пробелы/отступы
...
def _extract_final_reply(response, context_label: str = "") -> str:
    ...
    # Обрыв ВНУТРИ <think> (незакрытый тег): отрезать всё от него до конца,
    # иначе в ответ утечёт сырое рассуждение модели.
    if cleaned and "<think>" in cleaned and "</think>" not in cleaned:
        cleaned = cleaned.split("<think>")[0].strip()
    cleaned = _sanitize_reply(cleaned)

    # Совсем пусто — честный фолбэк.
    if not cleaned:
        logging.warning(
            f"[Kernel] Пустой ответ ({context_label}, finish_reason={finish_reason}) "
            f"— сырой текст: {raw!r}"
        )
        return _TRUNCATION_FALLBACK_MSG

    # Обрезано по длине, но осмысленный текст есть: показать его. Раньше здесь
    # ТЕРЯЛСЯ весь уже сгенерированный ответ (возвращалась заглушка «не успел»),
    # хотя модель реально ответила — просто длинно. В инциденте с матчем
    # пользователь из-за этого не увидел ответ, а на следующем вопросе модель
    # фантазировала, т.к. в истории не осталось реального ответа для опоры.
    if finish_reason == "length":
        logging.warning(
            f"[Kernel] Ответ обрезан по длине ({context_label}) — отдаю частичный "
            f"(длина {len(cleaned)})"
        )
        return cleaned.rstrip(" .…") + " … (ответ был длинным и обрезан — уточни, что именно интересно)"
...
async def call_model_router(
    ...
    system_prompt = (
        ...
        "ОСОБО про результаты поиска: если ты вызвал search_web, но в выдаче "
        "НЕТ прямого ответа на вопрос (например, конкретного счёта матча, "
        "цифры, даты, имени) — НЕ придумывай его. Прямо скажи, что нашёл общую "
        "информацию, но точного результата в источниках нет. НИКОГДА не "
        "выдумывай счёт, имена игроков, статистику или цитаты, которых нет в "
        "результатах поиска. Честное «не нашёл точный результат» лучше "
        "уверенного вымысла. "
